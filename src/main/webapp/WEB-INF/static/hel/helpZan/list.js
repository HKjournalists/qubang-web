   ...
   if(columnSize<=18){
	...
	manager = $("#maingrid").ligerGrid({
        columns: [
			          ...
			          { display: '用户ID', name: 'zanUserId', minWidth: 60,align:'left',
				        render:function(row){
				        		var html = '';
				        		html += '<a  href="javascript:void(0);" onclick="view(\'' + row.id + '\')">'+row.zanUserId+'</a>';
				        		return html;
				        	} 
					   }
...
var pagebar = null;
function loadData(){
	pagebar = $("#pagebar").pagebar({url:ctx+'/hel/helpZan/list.json',pageCallback:pageCallback,queryFormId:'helpZanQueryForm'});
}
...
function add() {
	//打开弹出窗口
	var options = {
		dialogId : "helpZanSaveDialog", //指定别名，自定义关闭时需此参数
		dialog : {
			title : "新增功能作者-待添加",
			height : dialogHeight,
            width : 500,
            showMax: true, 
			url : ctx+'/hel/helpZan/save.html?data='+ new Date(),
			buttons: [
...
function view(id) {
	//打开弹出窗口
	var options = {
		dialogId : "helpZanViewDialog", //指定别名，自定义关闭时需此参数
		dialog : {
			title : "查看功能作者-待添加",
			height : dialogHeight,
            width : 500,
            showMax: true, 
			url : ctx+'/hel/helpZan/view.html?id='+ id,
			buttons: [
                       { text: '关闭', onclick: function (item, dialog) { dialog.close(); } }
                    ]
		}
...
function edit() {
	...
	if (row) {
		//打开弹出窗口
		var options = {
			dialogId : "helpZanEditDialog", //指定别名，自定义关闭时需此参数
			dialog : {
				title : "编辑功能作者-待添加",
				height : dialogHeight,
	            width : 500,
				url : ctx+'/hel/helpZan/edit.html?id='+ row.id,
				buttons: [
                       { text: '保存', onclick: function (item, dialog) { dialog.frame.save();}},
                       { text: '关闭', onclick: function (item, dialog) { dialog.close(); } }
                    ]
...
function del() {
	...
	if (row) {
		window.parent.$.ligerDialog.confirm('确定删除?', function(flag) {
			if (flag) {
				$.get(ctx+"/hel/helpZan/delete.json?id="+row.id 
						, function(data) {
							if(data.success){
		        	    		$.notice("删除成功！","success");
		        	    		loadData();
