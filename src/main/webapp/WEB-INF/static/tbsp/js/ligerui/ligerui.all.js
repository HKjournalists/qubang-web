            ...
            else if (this.element.tagName.toLowerCase() == "select")
{
                ...
                if($(this.element).attr("validate")){
                	g.inputText.attr("validate", $(this.element).attr("validate"));
                	g.inputText.attr("name", g.textFieldID);
                }
...
{               
                ...
                if($(this.element).find("option:selected").val()){g.valueField[0].value = $(this.element).find("option:selected").val();}
            }
            if (p.valueFieldCssClass)
{
                g.valueField.addClass(p.valueFieldCssClass);
        ...
        */
        ...
        _showData: function ()
        {
            ...
            if (!data || !data.length)
            {
                ...
                $("<div>"+$.ligerDefaults.GridString.noRecordMessage+"</div>").addClass("l-grid-body-inner nodata").appendTo(g.gridbody);
        ...
        {
//        	if(rowdata && column.frozen){
        		return this._getRowDomId(rowdata, column.frozen) + "|" + column['__id'];
//        	}else{
//        		console.info("'frozen' can't find in the column  properties.");
//        	}
        },
...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addTabItem: function (options)
        {
            ...
            contentitem.attr("tabid", tabid); 
            var  tabData ="";
            if(options.data){tabData = options.data;}
            if (url)
            {
