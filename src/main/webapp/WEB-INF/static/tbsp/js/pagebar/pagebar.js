...
;(function($, window, document, undefined){  
    ...
    function Plugin(element, options){
        ...
        if(this.settings.queryFormId){
        	searchParamStr = $("#"+this.settings.queryFormId).serialize();
        }
        //console.log(searchParamStr);
        //拼接搜索条件
        //appName=aaa&appCname=sss&hostname=vvvv&webContext=555
        
        var params=searchParamStr.split("&");
        var paramStr = ""; 
        for(var i=0;i<params.length;i++){
        	var  fieldValue=params[i].split("=");
        	var field=fieldValue[0];
        	var value=fieldValue[1];
        	var stype=$("[name="+field+"]","#"+this.settings.queryFormId).attr("stype");
        	var sort=$("[name="+field+"]","#"+this.settings.queryFormId).attr("sort");
        	//console.log(stype);
        	if(stype){
        		paramStr+="search."+field+"_"+stype+"="+value+"&";
        		if(sort){
            		paramStr+="sort."+field+"="+sort+"&";
            	}
        	}
        }
